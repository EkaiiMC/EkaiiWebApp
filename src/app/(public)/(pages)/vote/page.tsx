        ...
        <div className="w-full overflow-y-auto max-h-[21.8rem] scrollbar-hide border-b-bgDarkGray border-b-[3px] mt-10">
          <table className="w-full text-left border-separate border-spacing-0">
            ...
            {leaderboard.map((entry, index) => (
              <>
                <tr key={entry.username} className={index === 0 ? 'sticky top-[30px] z-20 bg-basePink h-[48px]' : 'bg-bgLightGray'}>
                  <td className={index === 0 ? 'border-y-darkPink border-solid border-y-[3px] border-l-[3px] border-l-darkPink p-2.5' : 'p-2.5 border-solid border-l-[3px] border-l-bgDarkGray'}># {index + 1}</td>
                  <td className={index === 0 ? 'border-y-darkPink border-solid border-y-[3px] text-right relative' : 'text-right'}>
                    {index === 0 ? (
                      <Image src={'/images/first.svg'} alt="premier" width={40} height={40} className={'absolute top-1/2 transform -translate-y-1/2 right-2 w-[40px] h-[40px]'}/>
                    ) : index === 1 ? (
                  ...
                  <td className={index === 0 ? 'text-lg font-bold border-y-darkPink border-solid border-y-[3px]' : ''}>{entry.username}</td>
                  <td className={index === 0 ? 'text-lg font-bold border-y-darkPink border-solid border-y-[3px] border-r-[3px] border-r-darkPink' : 'border-solid border-r-[3px] border-r-bgDarkGray'}>{entry.voteCount}</td>
                </tr>
                {index !== 0 && index !== leaderboard.length - 1 && <tr className={'bg-bgLightGray'}>
                  <td className="border-solid border-l-[3px] border-l-bgDarkGray border-r-[3px] border-r-bgDarkGray" colSpan={4}>
